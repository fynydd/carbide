...
{
    /// <summary><![CDATA[
    /// The Email class provides a set of methods for sending and manipulating email.
    /// ]]></summary>
    /// <example>
    /// Make sure you add SMTP server information to your web.config file before using any email methods.
    /// In the Web.config file, only the "MailUseSsl" setting is used for the Email class. The other
    ...
    public static class Email
    {
        ...
        /// var attachmentNames = new ArrayList();
        /// var bytes = ms.ToArray();
        /// 
        /// attachments.Add(new MemoryStream(bytes, 0, bytes.Length));
        /// attachmentNames.Add("Certificate.pdf");
        /// 
        ...
        public static string Send(this UmbracoContext umbCtx, string senderAddress, string senderName, string recipient, string ccList, string bccList, string subject, string body, EmailFormat bodyFormat, ArrayList attachments, ArrayList attachmentNames)
        {
                ...
                if (attachments != null)
                {
                    if (attachments.Count > 0)
                    {
                        for (int x = 0; x < attachments.Count; x++)
                        {
                            ...
                            {
                                data = new System.Net.Mail.Attachment(umbCtx.MapPath((string)attachments[x]), MediaTypeNames.Application.Octet);
        ...
        /// <returns>"SUCCESS", or an error message</returns>
        public static string Send(this UmbracoContext umbCtx, string senderAddress, string senderName, string recipient, string subject, string body, EmailFormat bodyFormat, ArrayList attachments)
        {
            return umbCtx.Send(senderAddress, senderName, recipient, "", "", subject, body, bodyFormat, attachments, null);
        }
        ...
        /// <param name="bodyFormat">MailFormat value</param>
        /// <returns>"SUCCESS", or an error message</returns>
        public static string Send(this UmbracoContext umbCtx, string sender, string recipient, string subject, string body, EmailFormat bodyFormat)
        {
            ArrayList attachments = new ArrayList();
            return umbCtx.Send(sender, "", recipient, subject, body, bodyFormat, attachments);
        }
        ...
        /// <param name="bodyFormat">MailFormat value</param>
        /// <returns>"SUCCESS", or an error message</returns>
        public static string Send(this UmbracoContext umbCtx, string sender, string senderName, string recipient, string subject, string body, EmailFormat bodyFormat)
        {
            ArrayList attachments = new ArrayList();
            return umbCtx.Send(sender, senderName, recipient, subject, body, bodyFormat, attachments);
        }
        ...
        /// <param name="bodyFormat">MailFormat value</param>
        /// <returns>"SUCCESS", or an error message</returns>
        public static string Send(this UmbracoContext umbCtx, string sender, string senderName, string recipient, string ccList, string bccList, string subject, string body, EmailFormat bodyFormat)
        {
            ArrayList attachments = new ArrayList();
            return umbCtx.Send(sender, senderName, recipient, ccList, bccList, subject, body, bodyFormat, attachments, null);
        }
