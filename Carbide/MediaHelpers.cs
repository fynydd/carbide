...
namespace Fynydd.Carbide
{
    ...
    public static class MediaHelpers
    {
        ...
        public static string GetYoutubeEmbedCode(string youtubeUrlOrCode, string transport = "//")
        {
            ...
            if (!String.IsNullOrEmpty(youtubeUrlOrCode))
            {
                result = "<iframe class=\"youtube-video\" data-ytid=\"" + youtubeUrlOrCode + "\" id=\"yt" + Guid.NewGuid().ToString().Replace("-", "") + "\" src=\"" + GetYoutubeUrl(youtubeUrlOrCode, transport) + "?rel=0&autohide=2&modestbranding=1&theme=light&color=white&enablejsapi=1\" frameborder=\"0\" allowfullscreen></iframe>";
            }
