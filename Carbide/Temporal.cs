...
{
    /// <summary><![CDATA[
    /// The Temporal class contains methods and properties for manipulating, 
    ...
    public static class Temporal
    {
        ...
        /// <code><![CDATA[
        /// DateRangeStruct result = DateTime.Now.DateRange(Temporal.DateRangeOptions.Week);
        ...
        public static void TaskIntervalInit(this UmbracoContext umbCtx, string activityName, double seconds)
        {
            try
            {
                if (umbCtx.CacheExists(activityName + "_Seconds") == false || umbCtx.CacheExists(activityName + "_Running") == false)
                {
                    umbCtx.CacheAddPermanent(activityName + "_Running", false);
                    umbCtx.CacheAddPermanent(activityName + "_Seconds", seconds);
                    umbCtx.CacheDelete(activityName + "_Waiting");
        ...
        /// <param name="umbCtx">Umbraco context to use</param>
        /// <param name="activityName">Name of the activity, like "TwitterImport"</param>
        public static void TaskIntervalStart(this UmbracoContext umbCtx, string activityName)
        {
            try
            {
                umbCtx.CacheAddPermanent(activityName + "_Running", true);
                umbCtx.CacheDelete(activityName + "_Waiting");
        ...
        /// <param name="umbCtx">Umbraco context to use</param>
        /// <param name="activityName">Name of the activity, like "TwitterImport"</param>
        public static void TaskIntervalStop(this UmbracoContext umbCtx, string activityName)
        {
            try
            {
                umbCtx.CacheAddPermanent(activityName + "_Running", false);

                if (umbCtx.CacheExists(activityName + "_Seconds"))
                {
                    umbCtx.CacheAdd(activityName + "_Waiting", DateTime.Now.AddSeconds(umbCtx.Cache<double>(activityName + "_Seconds")), DateTime.Now.AddSeconds(umbCtx.Cache<double>(activityName + "_Seconds")));
                ...
                {
                    umbCtx.CacheDelete(activityName + "_Running");
                    umbCtx.CacheDelete(activityName + "_Seconds");
                    umbCtx.CacheDelete(activityName + "_Waiting");
        ...
        /// <param name="umbCtx">Umbraco context to use</param>
        /// <param name="activityName">Name of the activity, like "TwitterImport"</param>
        public static bool TaskIsRunning(this UmbracoContext umbCtx, string activityName)
        {
            bool result = false;

            if (umbCtx.CacheExists(activityName + "_Running") == true)
            {
                result = umbCtx.Cache<bool>(activityName + "_Running");
            }
        ...
        /// <param name="umbCtx">Umbraco context to use</param>
        /// <param name="activityName">Name of the activity, like "TwitterImport"</param>
        public static bool TaskShouldBeRun(this UmbracoContext umbCtx, string activityName)
        {
            bool result = true;

            if (umbCtx.CacheExists(activityName + "_Waiting") == false)
            {
                try
                {
                    if (umbCtx.TaskIsRunning(activityName) == true)
                    {
                        result = false;
            ...
            else
            {
                ...
                Debug.WriteLine("Carbide.Temporal.TaskShouldBeRun (" + activityName + ") - TOO SOON; " + FormatTimer((int)(Temporal.DateDiff<double>(DateTime.Now, umbCtx.Cache<DateTime>(activityName + "_Waiting"), DateDiffComparisonType.Seconds)), " ") + " to go");
            }
